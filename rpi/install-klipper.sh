...
if [ $? -ne 0 ]; then
    ...
    if [ ! -d $BASEDIR/klipper/ ]; then
        ...
        # additional packages for things like input shaper graphs, eddyng, cartographer, beacon, etc
        sudo apt-get install -y python3-numpy python3-matplotlib libatlas-base-dev libopenblas-dev || exit $?
