...
if [ $? -ne 0 ]; then
  echo "ERROR: Sudo requires a password - attempting to fix, you will be prompted for the $USER password"
  sudo -n true 2>/dev/null
  ...
  while true; do
    if [ "$1" = "--fix-client-variables" ] || [ "$1" = "--fix-serial" ] || [ "$1" = "--install" ] || [ "$1" = "--update" ] || [ "$1" = "--reinstall" ] || [ "$1" = "--clean-install" ] || [ "$1" = "--clean-update" ] || [ "$1" = "--clean-reinstall" ]; then
      ...
      if [ ! -f $BASEDIR/pellcorp.done ]; then
        ...
        existing_printer=$(cat $BASEDIR/pellcorp-overrides/config.info | grep printer= | awk -F '=' '{print $2}')
