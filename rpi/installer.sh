...
function install_config_updater() {
    ...
    if [ $? -ne 0 ]; then
        ...
        if [ $debian_release -ge 12 ]; then
            sudo pip install --break-system-packages configupdater==3.2
        else
...
function setup_bltouch() {
    ...
    if [ $? -ne 0 ]; then
        ...
        if [ -f $BASEDIR/pellcorp-backups/bltouch.factory.cfg ]; then
            $CONFIG_HELPER --file bltouch.cfg --overrides $BASEDIR/pellcorp-backups/bltouch.factory.cfg --quiet || exit $?
        fi
...
function setup_microprobe() {
    ...
    if [ $? -ne 0 ]; then
        ...
        if [ -f $BASEDIR/pellcorp-backups/microprobe.factory.cfg ]; then
            $CONFIG_HELPER --file microprobe.cfg --overrides $BASEDIR/pellcorp-backups/microprobe.factory.cfg --quiet || exit $?
        fi
...
function setup_klicky() {
    ...
    if [ $? -ne 0 ]; then
        ...
        if [ -f $BASEDIR/pellcorp-backups/klicky.factory.cfg ]; then
            $CONFIG_HELPER --file klicky.cfg --overrides $BASEDIR/pellcorp-backups/klicky.factory.cfg --quiet || exit $?
        fi
