...
# note this is no-op if there is no nozzle mcu
$CONFIG_HELPER --file $PRINTER_CFG --replace-section-entry "mcu nozzle_mcu" "baud" 230400 || exit $?
# note this is no-op if there is no leveling mcu
...
if [ "$MODEL" = "F002" ] || [ "$MODEL" = "F001" ]; then
  # the stock factory config's adxl345 sections are superseded by the
  # bed/nozzle sections printer.cfg.f001 adds below, but were never removed,
  # so both end up sharing the same cs_pin and Klipper refuses to start
...
sed -i 's/^pid_ki = /pid_ki: /g' $PRINTER_CFG
sed -i 's/^pid_kp = /pid_kp: /g' $PRINTER_CFG
sed -i 's/^pressure_advance = /pressure_advance: /g' $PRINTER_CFG

if [ -f $BASEDIR/k1/patches/fan_control.${model}.cfg ]; then
